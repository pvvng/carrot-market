...
import { purchasedInitialProducts } from "@/app/(tabs)/profile/purchase/page";
import { useEffect, useRef, useState } from "react";
import ListProduct from "./list-product";
import { getMorePurchasedProducts } from "@/app/(tabs)/profile/purchase/actions";

interface ProductListProps {
  initialProducts: purchasedInitialProducts;
}
    ...
    const observer = new IntersectionObserver(
      async (
        ...
        if (element.isIntersecting && trigger.current) {
          ...
          // 데이터 불러오기
          setIsLoading(true);
          const newProducts = await getMorePurchasedProducts(page + 1);
